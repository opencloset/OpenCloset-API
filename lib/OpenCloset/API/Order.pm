...
<%= $name %>님 <%= $datetime %>으로 방문 예약이 완료되었습니다.

<열린옷장 위치안내>
서울특별시 광진구 아차산로 213 웅진빌딩 5층.
건대입구역 1번 출구로 나와 오른쪽 다이소 방향으로 직진하여 국민은행 있는 건물 5층으로 와주세요(도보로 약 3분 소요)
...
<이용약속>
1. 코로나 19로부터 안전을 위해 방문 시 마스크착용 및 체온확인, 손소독제 사용을 부탁드립니다.
2. 지각은 NO! 노쇼 금지! 예약시간을 지켜주세요.

열린옷장은 <%= $name %>님의 멋진 모습을 위해 최선을 다할게요!

@@ employment-wing.txt
% my ($year) = @_;
<%= $year %> 취업날개 무료대여 안내

1. 대상 : 서울시 거주 중인 고교졸업예정자~ 만 39세 입사면접 예정자

2. 대여시 필수 지참서류

- 주민등록지가 서울인 경우 : 주민등록증 / 주민등록등본 / 운전면허증 (사진 증빙 가능)
- 주민등록지는 서울이 아니나 서울에 거주하는 경우 : 실거주 증명이 가능한 부동산계약서 / 수도전기 고지서 / 고시원,기숙사 입주확인서 / 택배 운송장 / 배달앱 이용내역 (본인 이름으로 된 증빙 만 인정)

매회 이용시 증빙이 필요하니 꼭 챙겨서 방문 부탁드립니다.
